...
export function ImprovementSuggestions({ suggestions }: ImprovementSuggestionsProps) {
  const getPriorityIcon = (priority: string) => {
    const iconMap: Record<string, JSX.Element> = {
      'high': <AlertCircle className="h-5 w-5 text-error" />,
      'medium': <AlertTriangle className="h-5 w-5 text-warning" />,
      'low': <InfoIcon className="h-5 w-5 text-info" />
    };
    
    return iconMap[priority] || <InfoIcon className="h-5 w-5 text-info" />;
  };
  ...
  const getPriorityBgColor = (priority: string) => {
    ...
    return colorMap[priority] || 'bg-info/10';
  };
  
  return (
    <Card className="bg-white shadow rounded-lg overflow-hidden">
      <CardHeader className="px-6 py-4 border-b border-gray-200">
        <CardTitle className="text-lg font-medium text-gray-900">Improvement Suggestions</CardTitle>
        <CardDescription className="text-sm text-gray-500">Actionable recommendations to improve your SEO tags</CardDescription>
      </CardHeader>
      <CardContent className="px-6 py-4">
        {suggestions.length > 0 ? (
          ...
          <div className="py-6 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Great job! Your SEO tags are well optimized.</p>
          </div>
        )}
      </CardContent>
    </Card>
